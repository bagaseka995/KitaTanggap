    ...
    <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div class="hidden md:flex gap-3 text-sm">
                <a href="{{ route('admin.relawan.index') }}" class="px-3 py-1 rounded bg-[#2E75B6] font-medium transition">Manajemen Relawan</a>
                <a href="{{ route('admin.penugasan.index') }}" class="px-3 py-1 rounded hover:bg-[#2E75B6]/50 transition">Penugasan Relawan</a>
...
<div class="max-w-7xl mx-auto px-4 py-8" x-data="relawanAdmin()" x-init="loadData()">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Manajemen Relawan</h1>

    {{-- Summary cards --}}
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 p-4 text-center shadow-sm">
            <p class="text-3xl font-bold text-gray-800" x-text="summary.total ?? '{{ $summary['total'] }}'"></p>
            <p class="text-xs text-gray-500 mt-1">Total</p>
        </div>
        <div class="bg-yellow-50 rounded-xl border border-yellow-200 p-4 text-center shadow-sm">
            <p class="text-3xl font-bold text-yellow-700" x-text="summary.pending ?? '{{ $summary['pending'] }}'"></p>
        ...
        <div class="bg-green-50 rounded-xl border border-green-200 p-4 text-center shadow-sm">
            <p class="text-3xl font-bold text-green-700" x-text="summary.terverifikasi ?? '{{ $summary['terverifikasi'] }}'"></p>
            <p class="text-xs text-green-600 mt-1">Terverifikasi</p>
        </div>
        <div class="bg-red-50 rounded-xl border border-red-200 p-4 text-center shadow-sm">
            <p class="text-3xl font-bold text-red-700" x-text="summary.ditolak ?? '{{ $summary['ditolak'] }}'"></p>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-5 flex flex-wrap gap-3">
        <select x-model="filter.status" @change="loadData()"
                class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#1F4E79]">
        ...
        <input x-model="filter.keahlian" @input.debounce.400ms="loadData()"
               ...
               class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#1F4E79] flex-1 min-w-32">
    ...
    {{-- Tabel --}}
    <div x-show="!loading" class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    <template x-if="relawan.length === 0">
                        <tr><td colspan="6" class="px-4 py-10 text-center text-gray-400">Tidak ada data relawan.</td></tr>
                    </template>
                    <template x-for="r in relawan" :key="r.id">
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                            <td class="px-4 py-3">
                                <p class="font-semibold text-gray-900" x-text="r.user.nama_lengkap"></p>
                                <p class="text-xs text-gray-400" x-text="r.user.email"></p>
                                <p class="text-xs text-gray-400" x-text="r.user.no_telepon"></p>
                            </td>
                            <td class="px-4 py-3 max-w-[180px]">
                                <div class="flex flex-wrap gap-1">
                                    <template x-for="k in r.keahlian_array" :key="k">
                                        <span class="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-xs" x-text="k"></span>
                                    </template>
                                    <span x-show="!r.keahlian_array || r.keahlian_array.length===0" class="text-gray-400 text-xs">—</span>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-gray-600" x-text="r.lokasi_domisili || '—'"></td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-gray-500 text-xs" x-text="r.created_at ? r.created_at.substring(0,10) : '—'"></td>
    ...
    <div x-show="dialog" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
            <h3 class="font-bold text-gray-900 text-lg mb-2">Konfirmasi Penolakan</h3>
            ...
            <div class="flex gap-3">
                <button @click="dialog=false" class="flex-1 py-2.5 border border-gray-300 rounded-xl text-sm font-medium hover:bg-gray-50 transition">Batal</button>
